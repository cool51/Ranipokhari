...
//    void scroll_callback(GLFWwindow *window,RaniPokhari *rani, double xoffset, double yoffset);
    ...
    void processInput(GLFWwindow *window,Camera *camera,Skybox *skybox);
...
void InputProcess::processInput(GLFWwindow *window, Camera *camera,Skybox *skybox) {
      float speed=0.2;
      if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)
            glfwSetWindowShouldClose(window, true);

      if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
            skybox->camera.ProcessKeyboard(FORWARD, speed);
      if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
            skybox->camera.ProcessKeyboard(BACKWARD, speed);
      if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
            skybox->camera.ProcessKeyboard(LEFT, speed);
      if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
            skybox->camera.ProcessKeyboard(RIGHT, speed);
